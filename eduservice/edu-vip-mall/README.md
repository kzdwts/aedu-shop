...
* [Apache Druid](http://www.apache-druid.cn/) 海量日志收集
    ...
    * 什么场景下应该使用Druid
        * 1、数据插入频率比较高，但较少更新数据
        * 2、大多数查询场景为聚合查询和分组查询（groupby），同事还有一定的检索与扫描查询
        * 3、将数据查询延迟目标定位100毫秒到几秒钟之间
        * 4、数据具有时间属性（Druid针对时间做了优化和设计）
        * 5、在多表场景下，每次查询仅命中一个大的分布式表，查询有可能命中多个娇小的lookup表
        * 6、场景中包含高级维度数据列（例如URL，用户ID等），并且需要对其进行快速计数和排序
        * 7、需要从kafka、HDFS、对象存储（入Amazon S3）中加载数据
